...
# Reuse existing nodebuilder-docmost container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-docmost; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-docmost -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
